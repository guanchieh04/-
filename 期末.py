...
if choice == '國泰: 2023.4.17 至 2025.4.16':
    df_original = pd.read_csv('future_KBar_CCF.csv')
    product_name = '國泰CCF'
...
if choice == '金融期貨FXF:2020.3.2至2025.4.14':
    df_original =pd.read_csv('future_KBar_FXF')
    product_name = '金融期貨FXF'
    df_original['time'] = pd.to_datetime(df_original['time'])  # 就能直接轉換
    df_original.columns
    df_original.head()
if choice == '台積電期貨CDF:2022.3.2至2025.4.14':
    df_original =pd.read_csv('future_KBar_CDF.csv')
    product_name = '台積電期貨CDF'
    df_original['time'] = pd.to_datetime(df_original['time'])  # 就能直接轉換
    df_original.columns
    df_original.head()
if choice == '小型臺指MXF:2020.3.2至2025.3.14':
    df_original =pd.read_csv('future_KBar_MXF.csv')
    product_name = '小型臺指MXF'
    df_original['time'] = pd.to_datetime(df_original['time'])  # 就能直接轉換
    df_original.columns
    df_original.head()
